...
class CommandExecutor:
    ...
    def __init__(self, ui_callback=None):
        """
        Args:
            ui_callback: function(action: str, data: dict) to notify UI
        """
        self.tts = TTSEngine()
        self.ui_callback = ui_callback or (lambda action, data: None)
    ...
    def execute(self, command: ParsedCommand):
        ...
        handler = handlers.get(command.category, self._handle_unknown)
        handler(command)

    def _handle_spotify(self, cmd: ParsedCommand):
        if cmd.action == "search_play":
            self.tts.speak(f"Playing {cmd.value}")
            self.ui_callback("spotify_search", {"query": cmd.value})
            ...
            level = int(cmd.value)
            self.tts.speak(f"Spotify volume {level} percent")
            self.ui_callback("spotify_volume", {"level": level})

    def _handle_volume(self, cmd: ParsedCommand):
        if cmd.action == "set":
            percent = int(cmd.value)
            set_volume(percent)
            self.tts.speak(f"Volume set to {percent} percent")
            self.ui_callback("volume_changed", {"level": percent})

        elif cmd.action == "up":
            current = get_volume()
            new_vol = min(100, current + int(cmd.value))
            set_volume(new_vol)
            self.tts.speak(f"Volume up to {new_vol} percent")
            self.ui_callback("volume_changed", {"level": new_vol})

        elif cmd.action == "down":
            current = get_volume()
            new_vol = max(0, current - int(cmd.value))
            set_volume(new_vol)
            self.tts.speak(f"Volume down to {new_vol} percent")
            self.ui_callback("volume_changed", {"level": new_vol})

        elif cmd.action == "mute":
            mute()
            self.tts.speak("Muted")
            self.ui_callback("volume_changed", {"level": 0, "muted": True})
            ...
            self.ui_callback(
                "volume_changed", {"level": current, "muted": False}
            )
    ...
    def _handle_maps(self, cmd: ParsedCommand):
        if cmd.action == "directions":
            self.tts.speak(f"Getting directions to {cmd.value}")
            self.ui_callback(
                "maps_directions", {"destination": cmd.value}
            )
        elif cmd.action == "search_nearby":
            self.tts.speak(f"Searching {cmd.value} nearby")
            self.ui_callback("maps_search", {"query": cmd.value})
    ...
    def _handle_whatsapp(self, cmd: ParsedCommand):
        ...
        if len(parts) == 2:
            ...
            self.tts.speak(f"Sending message to {contact}")
            self.ui_callback(
                "whatsapp_send",
    ...
    def _handle_translate(self, cmd: ParsedCommand):
        ...
        if len(parts) == 2:
            ...
            self.ui_callback(
                "translate", {"text": phrase, "target_lang": lang}
            )
        else:
            self.tts.speak("Couldn't understand translation request")
    ...
    def _handle_unknown(self, cmd: ParsedCommand):
        ...
        self.ui_callback("unknown_command", {"text": cmd.raw_text})