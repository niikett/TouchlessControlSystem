...
class AppButton(QPushButton):
    ...
    def __init__(self, name, icon_path, callback, parent=None):
        ...
        if not pixmap.isNull():
            ...
            icon_label.setText("?")
            icon_label.setStyleSheet("color: #888; font-size: 28px;")
...
class HomeScreen(QWidget):
    ...
    def __init__(self, on_app_open, parent=None):
        ...
        subtitle = QLabel("Gesture & Voice Controlled Interface")
        ...
        app_row = QHBoxLayout(app_bar)  # HBoxLayout = single row
        app_row.setSpacing(20)
        app_row.setContentsMargins(30, 20, 30, 20)
        app_row.setAlignment(Qt.AlignCenter)

        apps = [
            ("Google",   os.path.join(ICONS_DIR, "search.png"),   "search"),
            ("Maps",     os.path.join(ICONS_DIR, "maps.png"),     "maps"),
            ("WhatsApp", os.path.join(ICONS_DIR, "whatsapp.png"), "whatsapp"),
            ("Spotify",  os.path.join(ICONS_DIR, "spotify.png"),  "spotify"),
        ]

        for name, icon, screen in apps:
            btn = AppButton(
                name, icon, lambda checked, s=screen: self.on_app_open(s)
            )
            app_row.addWidget(btn)  # ✅ addWidget directly, no grid needed
...
class PlaceholderScreen(QWidget):
    ...
    def __init__(self, title, back_callback, parent=None):
        ...
        placeholder = QLabel(f"{title} module will be integrated here")
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.notif_label = QLabel("System ready — gesture control active")
        ...
        self.shortcut_esc = self._bind_key(Qt.Key_Escape, self.close)
    ...
    def _load_stylesheet(self):
        ...
        qss_path = os.path.join(
            os.path.dirname(__file__), "styles", "hud_theme.qss"
        )
        try:
    ...
    def _init_voice_system(self):
        ...
        self.voice_bridge.command_signal.connect(self._handle_voice_action)
        self.voice_bridge.status_signal.connect(self._handle_voice_status)
        self.voice_bridge.heard_signal.connect(self._handle_voice_heard)
    ...
    def _on_voice_command(self, text: str):
        """
        Called from voice BACKGROUND thread.
        Emit signal to safely update GUI on main thread.
        """
    ...
    def _handle_voice_action(self, action: str, data: dict):
        ...
        if action == "open_app":
            ...
            if search_screen and hasattr(search_screen, "perform_search"):
            ...
            if maps_screen and hasattr(maps_screen, "get_directions"):
            ...
            self.open_app("maps")
            maps_screen = self.screens.get("maps")
            if maps_screen and hasattr(maps_screen, "search_nearby"):
            ...
            if not success:
                ...
                self.set_notification("⚠️ Using Web Player (Premium required)")
            ...
            if muted:
                ...
                self.set_notification(f"🔊 Volume: {level}%")
            ...
            maps_screen = self.screens.get("maps")
            if maps_screen and hasattr(maps_screen, "start_navigation"):
    ...
    def go_home(self):
        ...
        self.notif_label.setText("System ready — gesture control active")
    ...
    def set_notification(self, message, level="info"):
        ...
        if level == "error":
            ...
            self.notif_bar.setObjectName("notifBar")
        self.notif_bar.setStyleSheet(self.notif_bar.styleSheet())
    ...
    def closeEvent(self, event):
        ...
        # Clean up web pages before closing
